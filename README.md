...
[1.9.0-beta.12](https://github.com/Asanilo/patina-Linux/releases/tag/v1.9.0-beta.12) is now available as a DEB prerelease; stable Latest remains 1.8.4. Local beta.9 acceptance covers owner handover, UI close/reopen, version reload, rollback/re-enable, backup export, remove/reinstall data retention, GNOME lock/suspend and Zen reconnection. Isolated remote restore now covers private Secret Service credentials, controlled HTTP download and real systemd restart for replace, merge and rollback; it does not establish third-party WebDAV/TLS interoperability. Desktop memory reclamation, active browser activity across suspend and the remaining installation failure matrix still need validation. AppImage daemon ownership/updating, Flatpak and the local browser UI are not ready. These are beta limitations, not completed stable-release guarantees.
