...
| `patinad` | Stage 2D audio preview | `--serve-api --track` runs tracking/watchdog, power lifecycle handling, and the PulseAudio/pipewire-pulse participation source for an isolated profile. Desktop remains the default owner; this is not a released background service. |
