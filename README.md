...
| `patinad` | Stage 2C power preview | `--serve-api --track` runs tracking/watchdog and observes lock, suspend, resume, and shutdown through systemd-logind for an isolated profile. Desktop remains the default owner; this is not a released background service. |
