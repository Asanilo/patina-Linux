...
- `main` 承担 Linux 日常开发与产品演进，已合入 daemon 分离，后台追踪由 `patinad` 拥有；原实验分支保留历史
...
当前 main 的本地存储位置与安全缓存控制已实现：活动数据库和 WebView 持久目录可独立迁移，迁移在重启时验证执行，旧目录保留；缓存维护只允许清理精确的 `WebKitCache`。受管 daemon 模式已沿既有存储向导完成宿主离线维护、跨进程资源屏障和中断恢复，不重复建设另一套 UI；候选验收范围见当前工作清单。

本地自动备份也已进入当前基线：支持按本机时间每天或每周执行，只写入用户选择的本机目录；文件采用不覆盖创建并在发布后重新解析校验，失败会有限重试，异常退出会在下次轮询对账。系统默认保留最近 3 份已验证且有数据库归属记录的快照，清理前必须再次核对目录、文件类型、大小和摘要；无法确认归属时保留文件。覆盖恢复会停用并换代原计划。显式 WebDAV 备份已进入 daemon 基线：密码存放于按 profile 隔离的系统凭据服务，上传从 daemon 自有 SQLite snapshot 完成；列表与恢复也由 daemon 有界读取、校验，并复用既有启动恢复状态机，不向 Desktop 暴露下载路径。自动上传 WebDAV 不属于当前基线，不能把手动配置隐式升级为后台任务。
...
| 工作线 | 目标 | 不做 |
| --- | --- | --- |
| Linux 产品主线 `main` | 日常开发、daemon 与 Desktop 维护、Linux 正确性与产品交付 | 扩张双 runtime owner，或宣称未验证的发行版支持 |
| `feature/patinad-daemon` | 保留分离阶段与 beta 开发历史，现已合入 main | 继续作为与 main 并行的完整产品线 |
...
- 下一步按模块评估贡献草稿中适用的 Linux provider、协议和测试，再将通过验证的实现移入 main；不整支合并上游历史，不因主线合流自动扩大贡献草稿。
...
beta.19 DEB 预发布已公开并核验；其后的修复已完成本地候选实装并合入本地 main，尚未公开发布。AppImage 持久运行时与原子更新已有隔离验证，真实首次接管、DEB 共存、登录及正式签名升级仍是该格式发布与稳定版的门槛。源码、安装和发布证据只维护在 [当前工作清单](./working/2026-07-10-patinad-runtime-design.md)，不在路线中追加每轮测试数字。
