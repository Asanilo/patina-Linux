...
15. Stage 2H.3 分三步完成默认切换：2H.3a 已完成 systemd 状态与迁移诊断；2H.3b.1/2 已完成安全 typed client，以及组合 `/current`、active session、SSE replay/resync 的只读 runtime adapter；2H.3b.3 继续接入显式 desktop client 模式；2H.3c 再迁移写侧、启用服务并完成双 owner 验收。默认切换前不得暴露会启动第二个 tracking owner 的服务开关。
