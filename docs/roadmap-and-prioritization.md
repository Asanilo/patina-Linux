...
15. Stage 2H.3 分三步完成默认切换：2H.3a 已完成 systemd 状态与迁移诊断；2H.3b.1/2 已完成安全 typed client，以及组合 `/current`、active session、SSE replay/resync 的只读 runtime adapter；2H.3b.3 已完成显式 `--daemon-client-preview` 接线、协议 v2 完整快照、embedded owner 隔离和真实 GNOME 桌面验收；2H.3c.1 已让 preview desktop 的 AFK threshold、tracking pause、audio participation、classification 与 Tools 读写通过 Rust host typed client 转发给 daemon，并把 Tools SSE 变更重新读取为前端既有完整 snapshot。2H.3c.2 已迁移 browser runtime、local API 配置和受白名单保护的普通 app settings；端口或 Token 变化会主动重建 typed client、SSE 与后续请求。2H.3c 后续仍需收口其他数据库写侧，再启用服务并完成默认 owner 切换验收。默认切换前不得暴露会启动第二个 tracking owner 的服务开关。
