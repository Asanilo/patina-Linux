...
2026-09-19 趋势批次：HTTP `/api/v1/trend` 已复用热力图的有界日仓储，按本地 DST 边界和当前分类排除策略返回总量/规范化 top app；桌面趋势缓存失效竞态已修。随后 Data **概览**已接通后端日总量，包含日/月图表、预热、缓存失效、失败重试和旧快照版本隔离；单次范围最多 378 天，超限明确报错。新 `/api/v1/activity/daily-apps` 已提供有界的每日全应用总量，并接通 typed daemon client、Desktop command 与前端校验适配器；查询级 5 万条合成事实对照通过。**应用/分类趋势页面仍读明细，旧分类全历史迁移仍未迁移**，不能把接口收口算成整个 Data 页低内存化。下一步处理显示名/分类元数据契约及页面接线，保留用户别名、分类、排除、日期跳转和导入统计语义，再补旧迁移与 AppImage。实测与检查状态见当前 working 文档。
