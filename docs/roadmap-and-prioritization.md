...
长期产品方向是 daemon/UI 分离和可替换客户端；上游贡献是独立的 Linux 兼容提案，不以整套 daemon 架构被上游接受为完成条件。两者都不承担追齐 Windows 全部功能的义务。

| 工作线 | 目标 | 不做 |
| --- | --- | --- |
| Linux 维护基线 `main` | 现有安装版必要的 Linux、安全及数据正确性修复 | 持续增长一套与 daemon 并行的完整产品 |
| `feature/patinad-daemon` | 后台唯一 owner、Desktop 客户端化、协议与生命周期验收 | 借重构继续增加通用统计、分类或界面功能 |
| 后续上游贡献分支 | 从届时最新上游选择性适配 Linux，提交小范围 PR | 携带 fork 全部差异、daemon 架构替换或自定义 UI/API |

贡献分支是阶段性工作，不是第三条长期产品发布线。当前不创建该分支，不合并 daemon 到 main，不自动脱离 fork、拆仓库或重命名。
...
beta.19 已推送并触发 DEB 预发布，不把触发工作流当成资产已公开。AppImage 持久运行时与原子更新已有隔离验证，真实首次接管、DEB 共存、登录及正式签名升级仍是该格式发布与稳定版的门槛。源码、安装和发布证据只维护在 [当前工作清单](./working/2026-07-10-patinad-runtime-design.md)，不在路线中追加每轮测试数字。
...
产品稳定后再决定是否让 daemon 成为 Linux 后续主线，避免永久维护双架构。KDE/wlroots 按 provider 能力逐个验证；Flatpak、Widget 闪烁/吸附、Windows 删除和 Cargo workspace 重排不作为分离的附带任务。暂停项未经用户恢复不重启。

### 5.7 上游 Linux 贡献线

后续用户启动时，从届时最新上游创建独立分支，先读贡献规则、核实平台边界并沟通维护者意愿。不先批量 cherry-pick 当前 fork，也不先投入整套架构迁移。
