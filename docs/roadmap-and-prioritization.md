...
2026-09-19 趋势批次：HTTP `/api/v1/trend` 已复用热力图的有界日仓储，按本地 DST 边界和当前分类排除策略返回总量/规范化 top app；桌面趋势缓存失效竞态已修。随后 Data **概览**已接通后端日总量，包含日/月图表、预热、缓存失效、失败重试和旧快照版本隔离；单次范围最多 378 天，超限明确报错。**应用/分类趋势仍读明细，旧分类全历史迁移仍未迁移**，不能把概览收口算成整个 Data 页低内存化。下一步优先设计应用/分类趋势需要的按日、按应用汇总契约，保留命名、分类、排除和导入统计语义，再补旧迁移与 AppImage。实测与检查状态见当前 working 文档。
