...
2026-09-19 趋势批次：HTTP `/api/v1/trend` 已复用热力图的有界日仓储，按本地 DST 边界和当前分类排除策略返回总量/规范化 top app；桌面趋势缓存失效竞态已修。**Data 趋势页面仍读明细，旧分类全历史迁移仍未迁移**，不能把接口收口算成所有趋势内存工作完成。下一步优先把桌面趋势需要的按日/应用汇总契约接通，保留命名、分类、日期范围和导入统计语义，再补旧迁移与 AppImage。实测与检查状态见当前 working 文档。
