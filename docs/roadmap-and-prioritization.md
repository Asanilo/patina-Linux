...
4. 下一阶段由 daemon 接管 tracking 与 watchdog，再按 power、audio、MPRIS、浏览器活动桥接的顺序迁移平台 owner。
5. 复用现有 React feature 建立由 `patinad` 在 loopback 提供的浏览器 UI，先覆盖 Dashboard、History、Data、当前会话和诊断。
6. 让 Tauri UI 成为 daemon 桌面客户端，保留 tray、通知、文件选择和 updater，验证关闭 UI 后继续记录、重开恢复和版本兼容诊断。
7. 完成 systemd user service、`.deb` / AppImage 安装差异、日志、升级和恢复验证。
8. 在 daemon 契约稳定后开发 TUI / CLI，并开始 KDE Wayland 适配；桌面端是否从 Tauri 迁往 GPUI 只按实测收益单独评估。
9. `patinad` 稳定后，单独分阶段删除冻结的 Windows 平台代码。
