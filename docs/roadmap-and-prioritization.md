...
12. Stage 2G Tools runtime preview 已完成：服务版本、协议上下限、write scope 协商、app mapping、classification、AFK threshold、tracking pause、运行中 browser/audio 配置，以及 Tools runtime owner、系统通知、SSE 与写侧 HTTP/MCP 已完成；API listener/token 管理和受控 service restart 仍待完成。完成后再让 Tauri 成为纯客户端；切换后不自动回退 embedded tracker，daemon 故障必须明确诊断并提供受控重启。
