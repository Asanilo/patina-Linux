...
12. Patina Desktop 所需的 daemon 写侧 API 和版本协商正分批实施：服务版本、协议上下限、write scope 协商，以及 app mapping、classification、AFK threshold、tracking pause 已完成；运行中 browser/audio 配置、Tools 写侧、API listener/token 管理和受控 service restart 仍待完成。完成后再让 Tauri 成为纯客户端；切换后不自动回退 embedded tracker，daemon 故障必须明确诊断并提供受控重启。
