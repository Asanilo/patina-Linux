...
当前执行位置：第 1 步已完成单向合流；第 2 步已完成 owner 审计和 fail-closed 防护，活动导入已通过 owner-only 暂存票据收口，定时备份也已由 daemon 持有唯一调度任务、配置 API、运行状态与 SSE 失效通知，按应用删除已通过受确认的事务 API 和 typed client 收口。第 3 步已移植启动恢复、采样恢复、watchdog 竞态、browser bridge 重试、网页趋势区间去重、power lifecycle generation、暂停原子边界，以及网页段与活动原生浏览器 session 的持久化事务绑定。第 4 步已完成受控恢复及 remote backup owner 收口：非密钥配置、Linux 系统凭据、上传、列表、有界下载和启动恢复衔接均已落地。第 5 步 Stage 2H.3d 已完成受限 systemd 控制基础、登录偏好的持久化语义拆分，以及两阶段 owner 交接的 owner-only reservation 状态机；当前进入 embedded 准备/重启与 daemon-client 激活衔接。设置/回滚入口只能在交接状态机完整接线后开放，最后执行 DEB 实机验收。详细安全顺序以 [`working/2026-07-10-patinad-runtime-design.md`](./working/2026-07-10-patinad-runtime-design.md) 为准。
...
15. Stage 2H.3 分阶段完成默认切换：2H.3a 已完成 systemd 状态与迁移诊断；2H.3b.1/2 已完成安全 typed daemon client 和只读 runtime adapter；2H.3b.3 已完成显式 preview、embedded owner 隔离和真实 GNOME 验收；2H.3c.1 至 2H.3c.7b 已完成 Desktop 写侧、活动导入、定时备份、按应用删除、受控恢复和 remote backup owner 收口。2H.3d.1 已完成固定 unit 的受限控制基础，2H.3d.2 已完成后台追踪与 Desktop 登录偏好的持久化拆分，2H.3d.3a 已完成 fail-closed owner cutover reservation 状态机；当前接入 embedded 准备/重启和 daemon-client 激活，之后才开放设置/回滚并执行 DEB 实机验收。默认交接完成前不得暴露会启动第二个 tracking owner 的服务开关。
