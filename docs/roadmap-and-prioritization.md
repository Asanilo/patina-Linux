...
2026-09-19 批次补充：流式导出、逐条目恢复解析、流式预览/校验已接入；5 万条合成数据对照显示导出和预览的 USS 峰值增量下降，流式预览的 debug 耗时增加。并发 SQLite 写入快照及目标冲突保留已有自动验证，详细数字和测试状态见工作文档。完整恢复 payload、真实成品体验、剩余趋势/分类迁移与 AppImage 仍未收口。下方 beta.17/18 候选过程为历史记录，不覆盖用户当前安装 beta.18 的确认。
