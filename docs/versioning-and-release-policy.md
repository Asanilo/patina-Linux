...
- `Patina_1.0.1_amd64.AppImage`
- `Patina_1.0.1_amd64.AppImage.tar.gz`
- `Patina_1.0.1_amd64.deb`
- `patina-gnome-shell-extension-vX.zip`

Linux updater 使用 Tauri 生成并签名的 `AppImage.tar.gz`；裸 AppImage 与 `.deb` 作为手动安装包，不作为 updater 下载目标。
...
`write-release-notes`、Windows NSIS 构建、Linux AppImage / `.deb` 构建、浏览器扩展打包与 release asset 准备
...
8. Windows job 生成 release notes、NSIS 安装包、Chromium 扩展与 Windows updater manifest，并先创建 GitHub Release。
9. Linux job 在 Ubuntu 22.04 上生成 AppImage、`.deb`、GNOME 扩展包和 Firefox / Zen XPI，再把 `linux-x86_64` 合并进同一个 `latest.json`。
10. GitHub Release 附件至少包含 Windows 安装包、Linux AppImage、Linux `.deb`、`latest.json`、Chromium 扩展包、GNOME 扩展包与 Firefox / Zen XPI。
11. 如果 R2 镜像 secrets 已配置，由工作流生成 Windows R2 版 `latest.json`、上传当前 Windows 安装包和 `latest.json`，并清理旧 R2 镜像；R2 暂不镜像 Linux 和浏览器扩展资产。
12. GitHub updater 清单同时服务 `windows-x86_64` 与 `linux-x86_64`。
