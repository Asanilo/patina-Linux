...
- 或已完成 `Publish Linux Release` 工作流对外发布
...
Linux updater 使用 Tauri 生成并签名的 `AppImage.tar.gz`；裸 AppImage 与 `.deb` 作为手动安装包，不作为 updater 下载目标。Windows 平台源码暂时保留，但当前发布线不生成、上传或承诺 Windows 安装包。

Linux 发布资产契约：

- `Patina_X.Y.Z_amd64.AppImage` 是可手动运行的便携包。
- `Patina_X.Y.Z_amd64.AppImage.tar.gz` 与对应 `.sig` 是 `latest.json` 使用的 Tauri updater 包。
...
Chromium zip 内部必须包含同名扩展目录，例如 `patina-chromium-extension-v0.1.0/`，并让 `README.en.md` 与 `README.zh-CN.md` 在 zip 根目录与该扩展目录并列，避免用户解压时把扩展运行文件散落到当前目录，同时方便先阅读使用说明。
...
`write-release-notes`、Linux AppImage / `.deb` 构建、浏览器扩展打包与 release asset 准备
默认属于 GitHub Actions 工作流 [`prepare-release.yml`](../.github/workflows/prepare-release.yml)
中的 `Publish Linux Release` 流程，只有在明确需要排查发布流水线问题时才例外。
