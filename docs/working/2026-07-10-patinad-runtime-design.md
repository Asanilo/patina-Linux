...
5. **2H.3d.5 DEB 成品与实机验收（进行中）**：覆盖首次迁移中断、重复执行、unit 缺失、systemd 不可用、服务崩溃、Token/端口不一致、旧 XDG autostart、pending storage migration 和自定义挂载目录。最后在已安装 DEB 上验证登录启动、关闭 UI 后持续记录、重开 UI、锁屏/睡眠、浏览器活动、升级、卸载与数据保留。
   ...
   - **2H.3d.5c 已安装包实机验收（进行中）**：只读验收采集器、旧版升级前基线、数据目录外的可恢复备份、`1.9.0-beta.1` DEB 安装、静态成品校验和完整 release gate 已完成。首次 owner 交接已达到 `completed`；关闭 Desktop 后 daemon 继续记录，重开 Desktop 未形成第二 owner；固定 service 崩溃后由 systemd 自动恢复；Firefox/Zen 扩展也已在 daemon 重启后重新连接浏览器桥接。`beta.1 → beta.2` 覆盖安装及受控 daemon 重启已确认 restart ticket、实例切换、数据库完整性和扩展重连；同时发现 completed reservation 的 Desktop 重开仍错误等待健康 daemon lease，已在 `beta.3` 源码中修复并有回归测试，待安装复验。剩余关口是 beta.3 Desktop 重开、锁屏/睡眠与音频/MPRIS 边界、后台登录偏好对账、设置页显式回滚与再次接管，以及卸载后的数据保留。
