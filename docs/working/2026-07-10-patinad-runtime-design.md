...
- [ ] **T6：按模块评估并选择性回流平台成果。** [当前执行记录](2026-09-21-linux-platform-reuse.md)中的采样中断、idle 可信性、GNOME 双协议及会话重绑已实现并通过完整门禁；version 4 扩展已在本机激活，用户已完成锁屏/挂起。beta.20 本机 DEB 升级、实际 AppImage/DEB 共存、无界面持续采样、重新登录后的会话恢复及真实系统重启后的后台自动启动均通过。AppImage `.DirIcon` 首次启动缺陷已修复，实际包与临时 systemd 故障恢复通过。剩余为纯 AppImage 安装用户的真实 systemd 接管、正式签名升级及 ESM/更多 Shell 验证；上游草稿仍保留，不整支合并。
...
| AppImage | beta.20 本地成品 daemon 入口、持久 AppDir、真实临时 systemd 本地/WebDAV 恢复、测试密钥验签替换通过；见 [第三片证据](2026-09-21-linux-platform-reuse.md#appimage-本地候选与证明范围) | 真实首次接管、DEB 共存、登录和正式升级验收完成；允许公开发布 |
