...
- 日仓储增加 `include_names=true` 可选契约，仅对有正贡献的 facts 在同一 read transaction 中按 256 个主键分批读取名称。名称与 exe 各限 1,024 UTF-8 bytes，名称计入既有 4 MiB 响应预算；不读出标题，不让被原生遮盖的导入或范围外记录提供名称。保留旧显示名打分与稳定首条 exe，helper alias 用 canonical identity。
- Desktop command 始终请求名称；前端验证 identities 与 daily keys 一一对应，旧 daemon 缺字段时报错而非回退本地 SQL。应用/分类趋势、预热及最多四范围缓存现在只保留日聚合，持久 bootstrap 增加 `appReadVersion=1`。用户覆盖和分类仍由既有分类模型解释；日/月统计、毫秒总量、详情入口保持。不把每天总量构造成虚假 session。
- 修正旧分类图表选择“空分类 + 有数据分类”时 series 索引错位：空分类保留零序列，后续类别不会占用其颜色/图例。跨午夜/月、365 天范围、37 ms、用户改名、helper alias、排除与空选择均有模型对照。新增真实浏览器 mock 下的错误/重试回归，失败时不显示旧成功图表；不等同安装版人工验证。
- `npm run check:full` 退出 0：627 项 Rust 测试通过、11 项 opt-in 忽略，38 项浏览器回归通过，Clippy、架构与 bundle 检查通过。index gzip 72.01 KiB，总 JS gzip 361.42 KiB，未提高预算。格式整理及 pending map 改名后对应模型/缓存测试复验通过。浏览器既有临时 profile `ENOTEMPTY` 警告不影响测试退出。
- `node scripts/perf/daily-activity-benchmark.mjs --daily-apps --names` 使用 50,000 条合成 native facts、30 天、单应用。旧全量领域快照 9,618 ms / USS 峰值增量约 372.0 MiB，新带名称日读 2,039 ms / 6.4 MiB；每天矩阵完全一致，总量 500,000,000 ms，带名称响应 3,899 bytes。证据 `/tmp/patina-daily-bench-SgTM45/summary.json`，fixture SHA-256 `dc066bf30d18dfa5d3a404c423bd3be725e1ba90a9c2cf47f36d2bb726474fe1`，读前后不变。这是 debug 查询进程，不是 WebKit/IPC 整链路或多应用极限证明。
- 没有版本变更、出包、安装、推送或发布；生产仍是 beta.18。旧分类全历史迁移和 AppImage 等剩余项目继续执行，不以本批完成宣称多 UI 前的门槛全部通过。
...
- owner：复用 `cmd_get_daily_activity` 及既有 Desktop/daemon 日查询适配，不新增另一套 SQL 或后台统计语义。前端只把每天的毫秒总量组合为日/月图表，保留均值和日期跳转，不伪造 session 区间。当前日读数是后端采样值，不承诺与前端渲染时钟精确到同一毫秒。
- `dataOverviewSnapshot` 最多保留四个范围，并合并同范围进行中请求；清理时递增 generation，旧请求不能重新填缓存或误删新请求。概览预热复用已有近期热力图日数据，不额外读取全年明细；应用趋势预热仍用旧 session 快照。持久 bootstrap 新增 `overviewReadVersion=1`，旧概览结果不会冒充新语义缓存。
- 热力图与概览共享日查询适配器；同一个 Desktop JS 上下文内串行发起，失败后释放队列。此措施不等于跨进程全局调度，也没有声称队列长度或取消行为已受控；后台既有并发/时间/facts 预算继续生效，外部调用竞争可报错。适配器按需加载，避免把新读取逻辑加入启动主入口。
- 请求失败、旧 daemon 不支持和超过 378 天范围均显示明确错误与重试，不返回截断数据，不回退到 SQL 明细。应用/分类趋势以及网页趋势未在本批改变；后续需要真正的 day/app 聚合契约，不能把总量拆成假应用数据。
- 验证范围：新增六项概览单元测试覆盖跨午夜/月、37 ms 精确总量、空午夜、去重、旧请求竞态、LRU、超限、失败恢复和本地串行队列；补旧概览 bootstrap 失效及浏览器错误/重试回归。浏览器采用隔离 mock，不等于 beta.18 已安装运行此改动，尚未测量这一批真实 Desktop PSS/USS 收益。
- `npm run check` 退出 0，37 项浏览器回归通过；1280/760 宽度的错误提示与恢复截图检查无重叠，证据在 `/tmp/patina-overview-ui-33SEpi/`。首次构建 index 超预算，改为按需加载后 index gzip 73.22 KiB、总 JS gzip 360.97 KiB，未提高预算。浏览器临时 profile 仍有既有 `ENOTEMPTY` 清理警告。本批仅前端与文档，不重复运行 Rust 全量测试；没有出包、安装、推送或公开发布。
