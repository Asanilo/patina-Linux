...
The release workflow publishes a Linux-only `latest.json` containing `linux-x86_64`. Updater delivery uses the signed AppImage generated by Tauri; the `.deb` remains the Debian / Ubuntu manual installation path.
