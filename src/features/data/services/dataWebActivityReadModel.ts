...
function compileSegments(
  ...
  const compiled = segments.flatMap((segment): CompiledWebTrendSegment[] => {
    ...
    return [{
      sourceKey: [
        segment.browserClientId,
        segment.browserKind,
        segment.browserExeName,
      ].join("\u0000"),
      normalizedDomain,
  ...
  return compiled.flatMap((segment): CompiledWebTrendSegment[] => {
    const key = `${segment.sourceKey}\u0000${segment.normalizedDomain}`;
    const startTime = Math.max(segment.startTime, sourceEnds.get(key) ?? segment.startTime);
    if (segment.endTime <= startTime) return [];
    sourceEnds.set(key, segment.endTime);
