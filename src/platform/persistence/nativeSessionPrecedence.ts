...
export function resolveNativeSessionPrecedence<T>(
  ...
  for (const candidate of buckets) {
    ...
    if (!Number.isFinite(capacityEndTime) || capacityEndTime <= candidate.record.startTime) continue;
...
function resolveExactRangesBySweep<T>(
  ...
  while (cursor < events.length) {
    ...
    while (exactHeap[0] && !activeExact.has(exactHeap[0].index)) heapPop(exactHeap);
...
function heapPop<T>(heap: IndexedRange<T>[]): IndexedRange<T> | undefined {
  ...
  while (true) {
    ...
    const right = left + 1;
    let smallest = index;
    if (left < heap.length && compareIndexedRanges(heap[left], heap[smallest]) < 0) smallest = left;
    if (right < heap.length && compareIndexedRanges(heap[right], heap[smallest]) < 0) smallest = right;
...
function intersectedDuration(intervals: TimeInterval[], startTime: number, endTime: number): number {
  return intervals.reduce((total, interval) => (
    total + Math.max(0, Math.min(endTime, interval.endTime) - Math.max(startTime, interval.startTime))
  ), 0);
}
