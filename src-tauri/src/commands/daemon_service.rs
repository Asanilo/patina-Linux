...
pub async fn cmd_retry_runtime_owner_cutover(
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        let reenable_from_rollback =
            runtime_mode.owns_embedded_runtime() && cutover.state == "rolled-back";
        if !runtime_mode.is_managed_daemon_client() && !reenable_from_rollback {
            return Err(
                "runtime owner cutover retry requires managed client or rolled-back embedded mode"
                    .to_string(),
            );
        }
        ...
        if reenable_from_rollback {
            crate::app::daemon_service::prepare_explicit_runtime_owner_reenable(
                profile,
                &control_root,
                settings,
            )
