...
- 新增只读 `/api/v1/activity/daily-apps` 和 Desktop/daemon 传输入口，按日返回所有规范化应用的时长；复用热力图统计规则，限制应用数、日/应用条目数及编码响应大小，超限不返回部分结果。字段级 OpenAPI/API 文档同步；应用/分类图表尚未切换到新入口。
- Data 概览趋势改用后端每日活动汇总，保留日/月图表、均值和日期跳转；失败或旧后台不支持时明确提示并允许重试，不回退会话明细。范围超过 378 天时明确拒绝，不截断统计。
- 概览预热复用热力图日数据，旧版概览持久快照失效；新增有界缓存和失效竞态保护。同一桌面窗口内的日查询串行执行，查询适配器按需加载。应用/分类趋势仍待迁移。
- HTTP 趋势改用热力图共享的有界日查询，按当前排除/进程过滤规则统计并合并 top_app 别名；修正跨夏令时日期边界，预算失败不返回部分统计。
- 修复桌面趋势缓存清理后，旧请求重新填充缓存或误删新请求去重状态的竞态；应用/分类趋势明细读取迁移仍在后续计划中。
...
- 分类页日常候选改用共享 Rust/daemon 有界聚合，只传应用汇总；保留别名、过滤与被排除应用的配置入口，旧版全历史分类迁移暂不改动。
