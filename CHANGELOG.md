...
- 修复本地 AppImage 首次启动时，构建目录的 `.DirIcon` 绝对链接触发持久解包拒绝的问题；补充实际包启动路由验收，AppImage 仍未恢复公开发布。
- 补充主线数据库升级、目录维护中断恢复、隔离 DEB 安装升级及原生界面重启验收，完善自动测试发现与架构边界检查。
- GNOME 扩展更新为 version 4，保留旧 D-Bus 方法/信号并提供 WindowTracker1；锁屏与 overview 隐去窗口事实，完善禁用、名称丢失与重启清理，保持 GNOME 42 支持范围。
- GNOME 采样端可识别新旧扩展协议，校验响应后才消费；新接口故障不降级成正常空窗口。扩展发行与更多 Shell 版本验收仍待完成。
- daemon 与 Desktop 开发已合流到 main；本版保持 DEB-only beta，AppImage 的实机接管、共存和正式升级门槛不变。

## [1.9.0-beta.19] - 2026-09-19

Release: Linux DEB 测试版：降低趋势、分类和备份的内存开销，完善低耗后台回收，修复窄窗口热力图与日期统计边界。
App note: 降低统计和备份内存开销，支持自定义后台回收延迟，修复热力图滚动。
App note en: Reduce statistics and backup memory use, customize background cleanup delay, and fix heatmap scrolling.

### Added

- 相对公开 beta.12，新增 1–60 分钟的低耗后台延迟设置，默认仍为 5 分钟；新增热力图、按日应用及分类候选只读 API，附字段级文档。

### Changed

- 热力图、概览和应用/分类趋势改用后端日汇总，分类候选与旧分类迁移不再向界面传输全量会话；保留名称、排除与导入优先级，查询超限明确报错，失败可重试。
- 减少启动重查询，按登录设置延迟创建窗口；开启低耗后台后，主窗口和 Widget 共用最后窗口回收，重开时取消旧回收任务。Linux 内存诊断采用 RSS/PSS/USS，不承诺固定内存上限。
- 备份导出、预览和校验采用流式读取写入，减少临时内存并保持界面响应；导出保留一致快照及原子发布，恢复仍在完整校验后单事务写入，不宣称完整恢复为恒定内存。

### Fixed

- 修复窄窗口热力图横向滚动与裁切，保留日期跳转；日统计正确处理夏令时和本地午夜边界。
- 修复最小化到 Widget、取消创建及自启动隐藏窗口的回收遗漏，避免 Wayland 未显示窗口的原生操作崩溃；原生 Wayland 使用自由拖动，闪烁与边缘吸附专项仍未完成。

### Internal

- 包含 beta.13–18 本地候选的累计变更；这些候选未单独公开发布。本次只发布 DEB 与扩展，不替换稳定版更新通道。
- AppImage 持久运行时、签名验证后原子替换、安装锁与预检已实现并通过隔离测试；真实首次接管、DEB 共存及升级验收未完成，因此不提供 AppImage 资产。
- 补充多表大库恢复回滚、内核写入失败、查询预算和真实 systemd 解析测试；自动验证不替代真实环境长期验收。
