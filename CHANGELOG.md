...
- 新增只读 `/api/v1/activity/daily-apps` 和 Desktop/daemon 传输入口，按日返回所有规范化应用的时长及可选名称；应用/分类趋势已接入，保留用户名称覆盖和分类，不再传输全年会话明细。字段级 OpenAPI/API 文档同步；读取超限不返回部分结果。
- Data 概览趋势改用后端每日活动汇总，保留日/月图表、均值和日期跳转；失败或旧后台不支持时明确提示并允许重试，不回退会话明细。范围超过 378 天时明确拒绝，不截断统计。
- 概览预热复用热力图日数据，旧版概览及应用趋势持久快照失效；新增有界缓存和失效竞态保护。同一桌面窗口内的日查询串行执行，查询适配器按需加载。
- HTTP 趋势改用热力图共享的有界日查询，按当前排除/进程过滤规则统计并合并 top_app 别名；修正跨夏令时日期边界，预算失败不返回部分统计。
- 修复桌面趋势缓存清理后，旧请求重新填充缓存或误删新请求去重状态的竞态，以及空分类选择导致图表 series 索引错位的问题。
- 完整备份恢复复用流式 CRC/JSON 读取器，避免整段条目 JSON 与解析 payload 同时驻留；保留全部校验完成后单事务写入。新增 5 万条记录的 Merge/Replace 恢复峰值与晚期写入失败回滚基准。
- 备份预览合并进行中的重复请求，公共异步归档检查限制进程内单 worker；取消调用方不会提前释放仍在运行的检查许可。增加 release 分阶段性能测量、隔离追踪写入和慢预览页面响应验证。
- 备份预览与定时/远端校验改为逐记录验证计数，减少完整恢复数据驻留；校验与 SHA-256 复用同一文件句柄，耗时预览及主要异步校验调用移入阻塞工作线程。
- 增加合成大库备份性能 runner、并发写入快照一致性及目标冲突保留测试。流式预览以更低内存换取一定解析耗时，不改变恢复策略。
- 恢复备份时逐条目校验、解析并释放原始 JSON，避免同时保留所有条目文本；晚期条目失败仍不写入恢复数据。
- 备份恢复读取直接使用磁盘 ZIP，去掉完整归档内存副本；保留格式与校验，并补实际文件类型、大小和 Unix 安全打开检查。
- 备份导出在单一 SQLite 快照中逐行写入私有临时 ZIP，写完同步后原子发布；定时备份不覆盖已有目标，导出失败清理本次临时文件。完整恢复仍持有解析后的 payload，尚非整链路低内存实现。
- 分类页日常候选改用共享 Rust/daemon 有界聚合，只传应用汇总；保留别名、过滤与被排除应用的配置入口。旧分类全历史迁移也改为后端分组流式读取，完整读取成功后才写入迁移标记，不截断为最近一年。
- 新增只读 `/api/v1/classification/observed-apps` 及字段级 OpenAPI；读取超限不返回截断统计，分类加载失败支持明确重试。

- AppImage 开发包包含 daemon 与完整持久运行时，固定 systemd user unit 不依赖临时挂载；保留 DEB 单 owner 及自定义 unit/mask，不自动降级运行时。通过独立候选验收前仍维持 DEB-only beta 发布。
- AppImage 验签成功后采用同目录原子替换，保留旧包；失败不退回原地覆盖，不改变公钥或 DEB 更新路径。

### Internal

- 增加多表大库备份/恢复、内核写入失败、AppImage 暂存及版本切换、Tauri 验签和持久 AppDir 下 systemd 本地/远端恢复隔离测试；测试与真实安装验收分开记录。

## [1.9.0-beta.18] - 2026-09-18

Release: 桌面内存优化候选：支持自定义低耗后台延迟，修复窄窗口热力图滚动，并延续后端日汇总和窗口回收优化。
App note: 可自定义低耗后台等待时间，修复窄窗口热力图滚动。
App note en: Customize the background cleanup delay and fix heatmap scrolling in narrow windows.

### Added

- 设置新增“低耗后台延迟”，支持 1–60 分钟，默认仍为 5 分钟；关闭低耗开关保留延迟值，不改变后台追踪或页面返回首页的计时。
- 包含 beta.17 的只读 `/api/v1/heatmap` 与后端日汇总，桌面热力图不再传输或缓存全年会话明细；日期、排除、导入优先级和旧 daemon 提示保持一致。

### Changed

- 延续 beta.13–17 的启动重查询精简、登录启动延迟建窗和最后 WebView 空闲堆回收，Linux 内存诊断使用 RSS/PSS/USS；不承诺整个桌面的固定内存上限。

### Fixed

- 修复 Data 热力图在 900px 及以下窄窗口中被裁切、横向滚动范围不足的问题；滚动区域保持在面板内，月份与日期列对齐。
- 修改低耗策略或重开主窗口会使旧销毁计时失效；最终可见性检查与销毁在 UI 线程执行。包含此前窗口生命周期、Wayland 崩溃及重复定位修复；悬浮窗闪烁与边缘吸附仍暂停。

### Internal

- 新增隔离 Wayland 原生延迟验收入口，覆盖一分钟回收、提前重开、关闭低耗开关及修改时长。不把原生窗口测试当作长期追踪或完整 React 页面验收。

## [1.9.0-beta.17] - 2026-09-18

Release: 桌面内存优化 DEB 候选：热力图采用后端日汇总，减少启动重查询，完善后台窗口回收与统计边界。
App note: 减少热力图和启动查询开销，完善后台回收与跨日统计。
App note en: Reduce heatmap and startup query overhead; improve background cleanup and day-boundary accounting.

### Added

- 本地 API 新增只读 `/api/v1/heatmap`，提供按本地日期汇总的数据与字段级 OpenAPI 说明，不返回标题或 URL。

### Changed

- Data 热力图与预热只接收和缓存日汇总，不再将 53 周逐条 session 经 SQLite/IPC 搬入桌面页面；桌面内置与 daemon 模式共用后端查询规则。
- 日聚合限制查询范围、并发和读取预算；失败时显示错误并支持重试，旧 daemon 显示更新提示，不静默回退全量明细查询。
- 相对已发布 beta.12，取消 Dashboard 启动时的全年热力图和分类候选重查询；登录启动按设置创建窗口，Widget 模式不再预建隐藏主窗口。

### Fixed

- 热力图应用当前应用排除设置和别名规则；保留本机记录优先于导入数据的规则，按每天实际交集分配导入小时桶。
- 热力图按本地日历午夜切分，修正夏令时和历史时区变更下的日期边界，并使旧统计缓存失效。
- 主窗口允许通知插件读取权限状态，避免初始化时出现未处理错误；不增加发送通知或申请权限能力。
- 汇入 beta.13–16 的后台生命周期修复：最小化到 Widget 纳入低耗回收、取消创建后清理隐藏窗口、最后 WebView 销毁后尝试归还空闲堆；默认开关和五分钟等待不变。
- 汇入 Wayland 原生窗口崩溃与重复定位修复；原生 Wayland 使用自由拖动，不宣称支持边缘吸附，悬浮窗闪烁专项仍暂停。

### Internal

- 新增共享统计 fixture、查询预算回归及隔离 React/WebKit/daemon 性能验收。已验证合成数据下的响应体积与窗口销毁流程，不承诺整个桌面的固定内存上限；其他趋势查询和悬浮窗视觉问题不在本次范围内。
- Linux 内存诊断沿用 beta.15 的 RSS/PSS/USS/Swap 口径，兼容 private usage 字段使用 USS，不再将 VmData 当作私有驻留内存。下方 beta.13–16 条目保留本地候选沿革，不代表这些版本已公开发布。
...
- 后台登录启动偏好已通过关闭和重新启用的双向实机验收：reservation、SQLite 镜像与 systemd enable 状态一致收敛，当前 daemon 不被误停或重启。
