...
仍需要按 Linux 架构独立评估的上游方向：

- 启动来源与窗口首帧生命周期，以及开机自启和静默启动解耦。
- 活动导入、应用/网站详情、网页/分类趋势和定时备份/导出。
- 跨页面快捷分类、累计提醒、挂件常驻状态和侧栏文字模式。
- 本地化、质量门禁和读模型工程整理，需与 Linux API、诊断、扩展和 `patinad` 路线一起评估。
