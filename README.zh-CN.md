...
| `patinad` | Stage 2D 音频参与预览 | 显式使用 `--serve-api --track` 后，daemon 可为隔离 profile 运行 tracking/watchdog、电源生命周期处理和 PulseAudio/pipewire-pulse 音频参与信号。桌面端仍是默认 owner，当前不作为正式后台服务发布。 |
