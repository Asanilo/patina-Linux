...
| `patinad` | Stage 2C 电源生命周期预览 | 显式使用 `--serve-api --track` 后，daemon 可为隔离 profile 运行 tracking/watchdog，并通过 systemd-logind 处理锁屏、休眠、恢复和关机封口。桌面端仍是默认 owner，当前不作为正式后台服务发布。 |
